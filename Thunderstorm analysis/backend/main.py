...
def get_cwc_historical_dates(file_name: str = None):
    ...
    for r in db:
        ...
        if not db_time or db_time == "None" or db_time == "":
            std_time = "05:00 IST (Operational Cycle)"
    ...
    dates.sort(
        key=lambda x: (
            x["date"],
            1 if str(x.get("time", "")).startswith("17:00") else 0,
        ),
        reverse=True,
    )
    return dates
...
def get_cwc_historical_analysis(
    ...
    return {
        ...
        "observation_timestamp": f"{date} " + (
            "17:00 IST" if str(matching_row.get("time", "")).strip().upper().startswith("12")
            else "05:00 IST"
        ),
    ...
    try:
        ...
        def clean_time(value, fallback_index=0):
            ...
            if "05:00" in upper or "00Z" in upper or upper == "00":
                return "05:00 IST"
            return "17:00 IST" if fallback_index % 2 else "05:00 IST"
